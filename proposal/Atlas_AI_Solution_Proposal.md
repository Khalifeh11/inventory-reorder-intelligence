...
- Plain-language "why" explanation for each recommendation
...
### 4.3 Plain-language explanations

Every reorder recommendation is paired with a 2–3 sentence plain-language "why". When the server is online, these are generated by an AI language model with guardrails that prevent it from inventing numbers — it can only restate what the reorder engine calculated. When the server is offline or unreachable, the same explanation is produced from a built-in template. Both paths show the same underlying numbers, and the dashboard labels which one was used.

This keeps the benefit (an explanation a non-technical user can read) without making the system dependent on internet connectivity — a realistic concern given Atlas's infrastructure.
...
4. **Explain** — each flagged product gets a 2–3 sentence plain-language "why".
...
- **Explanation guardrails.** The AI explanation path is constrained to restate only numbers the reorder engine produced — it cannot invent new figures. The dashboard labels every explanation with its source so the reviewer knows whether they're reading AI-generated or template text.
...
| AI boundary defined | ✅ | Used only for plain-language explanations, with an offline template fallback |
...
- **Dashboard app** — overview of products to reorder, per-product drilldown with forecast chart, what-if analysis
